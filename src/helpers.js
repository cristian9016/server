...
hbs.registerHelper('obtenerCursos', (coord) => {
    ...
    let text = '<table border=1><thead><th>NOMBRE</th><th>DESCRIPCION</th><th>VALOR</th><th>DETALLES</th></thead><tbody>';
    let disponibles = cursos.filter(cur => cur.estado == 'disponible');
    disponibles.forEach(curso => {
        text = text + '<tr><td>' + curso.nombre + '</td><td>' + curso.descripcion + '</td><td>' + curso.valor + '</td><td><form action="detalle" method="GET"><input type="hidden" name="id" value="' + curso.id + '"><input type="hidden" name="coord" value="' + coord + '"><button>VER</button></form></td><tr>';
    });
...
hbs.registerHelper('obtenerInscritos', (id) => {
    ...
    let text = '<table border=1><thead><th>NOMBRE</th><th>IDENTIFICACION</th><th>CORREO</th><th>TELEFONO</th><th></th></thead><tbody>';
    curso.matriculados.forEach(est => {
        ...
        '<input type="hidden" name="identificacion" value="'+est.identificacion+'"><input type="hidden" name="id" value="'+id+'"><button>Eliminar</button></form></td><tr>';
    });
    text = text + '</tbody></table>';
